...
struct PageSession {
    explicit PageSession(u32 id, Core::AnonymousBuffer theme, int width, int height)
        ...
        , view(BridgeView::create(move(theme), { viewport_width, viewport_height }))
    {
        ...
        view->on_load_start = [this](URL::URL const& url, bool) {
            loading = true;
            progress_percent = 15;
            pending_url = url.serialize().to_byte_string();
            crashed = false;
            crash_reason = {};
            metrics_dirty = true;
            mark_dirty();
        ...
        view->on_load_finish = [this](URL::URL const& url) {
            ...
            committed_url = url.serialize().to_byte_string();
            pending_url = {};
            metrics_dirty = true;
        ...
        view->on_web_content_crashed = [this] {
            ...
            crashed = true;
            progress_percent = 0;
            crash_reason = ByteString { "Ladybird WebContent crashed" };
            pending_url = {};
            mark_dirty();
        };
    }
    ...
    void refresh_metrics(bool force = false)
    {
        ...
        if (auto value = object.get_string("url"sv); value.has_value() && !value->is_empty()) {
            if (loading)
                pending_url = value->to_byte_string();
            else
                committed_url = value->to_byte_string();
        }
    ...
    bool load_markup(ByteString const& base_url, ByteString const& markup)
    {
        ...
        progress_percent = 20;
        pending_url = {};
        committed_url = base_url.is_empty() ? ByteString { "about:blank" } : base_url;
    ...
    void fill_state(RinWebContentPageState& state)
    {
        ...
        state.viewport_width = static_cast<u32>(max(viewport_width, 0));
        state.viewport_height = static_cast<u32>(max(viewport_height, 0));
        state.content_width = static_cast<u32>(max(content_width, viewport_width));
        state.content_height = static_cast<u32>(max(content_height, viewport_height));
...
static int handle_resize(PageSession& page, ReadonlyBytes payload)
{
    ...
    page.viewport_width = max(static_cast<int>(request.viewport_width), 1);
    page.viewport_height = max(static_cast<int>(request.viewport_height), 1);
    page.view->reset_viewport_size({ page.viewport_width, page.viewport_height });
...
static ErrorOr<int> run_bridge()
{
    ...
    if (theme_or_error.is_error()) {
        ...
        rin_log(message.characters());
        auto error = theme_or_error.release_error();
        cleanup_server_socket();
        return error;
    }
