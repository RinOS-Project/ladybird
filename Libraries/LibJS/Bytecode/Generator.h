...
public:
    ...
    static Vector<Operand> materialize_operand_vector(ReadonlySpan<ScopedOperand> operands)
    {
        Vector<Operand> materialized;
        materialized.ensure_capacity(operands.size());
        for (auto const& operand : operands)
            materialized.unchecked_append(operand.operand());
        return materialized;
    }

    static Vector<Optional<Operand>> materialize_optional_operand_vector(ReadonlySpan<Optional<ScopedOperand>> operands)
    {
        Vector<Optional<Operand>> materialized;
        materialized.ensure_capacity(operands.size());
        for (auto const& operand : operands) {
            if (operand.has_value())
                materialized.unchecked_append(operand->operand());
            else
                materialized.unchecked_append({});
        }
        return materialized;
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::NewArray>)
    void emit(Operand dst, ReadonlySpan<ScopedOperand> elements)
    {
        auto operand_elements = materialize_operand_vector(elements);
        emit_with_extra_slots<OpType, Operand>(operand_elements.size(), dst, operand_elements.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::NewArray>)
    void emit(Operand dst, Vector<ScopedOperand> const& elements)
    {
        emit<OpType>(dst, ReadonlySpan<ScopedOperand> { elements.data(), elements.size() });
    }
    ...
    template<typename OpType>
    requires(IsSame<OpType, Op::Call>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Operand this_value, Optional<StringTableIndex> expression_string, ReadonlySpan<ScopedOperand> arguments)
    {
        VERIFY(extra_operand_slots == arguments.size());
        auto operand_arguments = materialize_operand_vector(arguments);
        emit_with_extra_slots<OpType, Operand>(extra_operand_slots, dst, callee, this_value, expression_string, operand_arguments.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::Call>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Operand this_value, Optional<StringTableIndex> expression_string, Vector<ScopedOperand> const& arguments)
    {
        emit_with_extra_operand_slots<OpType>(extra_operand_slots, dst, callee, this_value, expression_string, ReadonlySpan<ScopedOperand> { arguments.data(), arguments.size() });
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::GetTemplateObject>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, FlatPtr cache, ReadonlySpan<ScopedOperand> strings)
    {
        VERIFY(extra_operand_slots == strings.size());
        auto operand_strings = materialize_operand_vector(strings);
        emit_with_extra_slots<OpType, Operand>(extra_operand_slots, dst, cache, operand_strings.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::GetTemplateObject>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, FlatPtr cache, Vector<ScopedOperand> const& strings)
    {
        emit_with_extra_operand_slots<OpType>(extra_operand_slots, dst, cache, ReadonlySpan<ScopedOperand> { strings.data(), strings.size() });
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CallBuiltin>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Operand this_value, Builtin builtin, Optional<StringTableIndex> expression_string, ReadonlySpan<ScopedOperand> arguments)
    {
        VERIFY(extra_operand_slots == arguments.size());
        auto operand_arguments = materialize_operand_vector(arguments);
        emit_with_extra_slots<OpType, Operand>(extra_operand_slots, dst, callee, this_value, builtin, expression_string, operand_arguments.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CallBuiltin>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Operand this_value, Builtin builtin, Optional<StringTableIndex> expression_string, Vector<ScopedOperand> const& arguments)
    {
        emit_with_extra_operand_slots<OpType>(extra_operand_slots, dst, callee, this_value, builtin, expression_string, ReadonlySpan<ScopedOperand> { arguments.data(), arguments.size() });
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CallConstruct>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Optional<StringTableIndex> expression_string, ReadonlySpan<ScopedOperand> arguments)
    {
        VERIFY(extra_operand_slots == arguments.size());
        auto operand_arguments = materialize_operand_vector(arguments);
        emit_with_extra_slots<OpType, Operand>(extra_operand_slots, dst, callee, expression_string, operand_arguments.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CallConstruct>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Optional<StringTableIndex> expression_string, Vector<ScopedOperand> const& arguments)
    {
        emit_with_extra_operand_slots<OpType>(extra_operand_slots, dst, callee, expression_string, ReadonlySpan<ScopedOperand> { arguments.data(), arguments.size() });
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CallDirectEval>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand callee, Operand this_value, Optional<StringTableIndex> expression_string, ReadonlySpan<ScopedOperand> arguments)
    {
        VERIFY(extra_operand_slots == arguments.size());
        auto operand_arguments = materialize_operand_vector(arguments);
    ...
    template<typename OpType>
    requires(IsSame<OpType, Op::CopyObjectExcludingProperties>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand from_object, ReadonlySpan<ScopedOperand> excluded_names)
    {
        VERIFY(extra_operand_slots == excluded_names.size());
        auto operand_names = materialize_operand_vector(excluded_names);
        emit_with_extra_slots<OpType, Operand>(extra_operand_slots, dst, from_object, operand_names.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::CopyObjectExcludingProperties>)
    void emit_with_extra_operand_slots(size_t extra_operand_slots, Operand dst, Operand from_object, Vector<ScopedOperand> const& excluded_names)
    {
        emit_with_extra_operand_slots<OpType>(extra_operand_slots, dst, from_object, ReadonlySpan<ScopedOperand> { excluded_names.data(), excluded_names.size() });
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::NewClass>)
    void emit_with_extra_slots(size_t extra_slot_count, Operand dst, Optional<Operand> super_class, Operand class_environment, u32 class_blueprint_index, Optional<IdentifierTableIndex> lhs_name, ReadonlySpan<Optional<ScopedOperand>> element_keys)
    {
        VERIFY(extra_slot_count == element_keys.size());
        auto operand_keys = materialize_optional_operand_vector(element_keys);
        emit_with_extra_slots<OpType, Optional<Operand>>(extra_slot_count, dst, super_class, class_environment, class_blueprint_index, lhs_name, operand_keys.span());
    }

    template<typename OpType>
    requires(IsSame<OpType, Op::NewClass>)
    void emit_with_extra_slots(size_t extra_slot_count, Operand dst, Optional<Operand> super_class, Operand class_environment, u32 class_blueprint_index, Optional<IdentifierTableIndex> lhs_name, Vector<Optional<ScopedOperand>> const& element_keys)
    {
        emit_with_extra_slots<OpType>(extra_slot_count, dst, super_class, class_environment, class_blueprint_index, lhs_name, ReadonlySpan<Optional<ScopedOperand>> { element_keys.data(), element_keys.size() });
    }
