...
void OpenGLContext::fail_rin_gl_surface(int result)
{
    ...
    // A malformed/failed surface or bridge is not evidence that the native
    // device disappeared. Do not synthesize a WebGL context loss for that
    // case; keep the context unavailable and report the normal realization
...
{
    m_impl->half_float_texture_linear_enabled = true;
    make_current();
    if (m_impl->bridge.context
        && ringl_enable_webgl_half_float_texture_linear() != 0)
        fail_rin_gl_surface(RINGL_AQUAMARINE_SURFACE_STATE);
...
{
    m_impl->float_color_buffer_enabled = true;
    make_current();
    if (m_impl->bridge.context
        && ringl_enable_webgl_float_color_buffer() != 0)
        fail_rin_gl_surface(RINGL_AQUAMARINE_SURFACE_STATE);
...
{
    m_impl->half_float_color_buffer_enabled = true;
    make_current();
    if (m_impl->bridge.context
        && ringl_enable_webgl_half_float_color_buffer() != 0)
        fail_rin_gl_surface(RINGL_AQUAMARINE_SURFACE_STATE);
...
{
    m_impl->standard_derivatives_enabled = true;
    make_current();
    if (m_impl->bridge.context
        && ringl_enable_webgl_standard_derivatives() != 0)
        fail_rin_gl_surface(RINGL_AQUAMARINE_SURFACE_STATE);
