...
void AudioContext::render_audio(Span<float> buffer)
{
    ...
    for (size_t frame = 0; frame < frame_count; ++frame) {
        ...
        // Collect direct source -> AudioParam edges before rendering any
        // source.  The old one-pass traversal made the result depend on the
        // order of m_active_* vectors: a later source could not modulate a
        // parameter that an earlier source had already evaluated.  This
        // bounded pre-pass gives every direct source edge the same frame
        // snapshot, while the normal traversal still handles intermediate
        // node edges and stateful DSP exactly once.
                    ...
                    for (auto const& param_connection : m_param_connections) {
                        if (param_connection.source_node_id == source_node_id && param_connection.output_index == 0)
                            add_param_modulation(param_connection.destination_param_id,
                                ((source_left - precollected.left) + (source_right - precollected.right)) * 0.5f);
                    }
                    break;
                }
            }
