...
void fetch_classic_script(GC::Ref<HTMLScriptElement> element, URL::URL const& url, EnvironmentSettingsObject& settings_object, ScriptFetchOptions options, CORSSettingAttribute cors_setting, String character_encoding, OnFetchScriptComplete on_complete)
{
    ...
    fetch_algorithms_input.process_response_consume_body = [&settings_object, options = move(options), character_encoding = move(character_encoding), on_complete = move(on_complete)](auto response, auto body_bytes) {
        // Root response and internal_response so they survive any GC cycle
        // that may be triggered by allocations later in this callback.
        // The conservative stack scanner may miss register-only temporaries
        // in optimised builds, so explicit roots are required.
        auto response_root = GC::make_root(response);
        auto internal_response = response->unsafe_response();
        auto internal_response_root = GC::make_root(internal_response);

        // 2. If either of the following conditions are met:
        // - bodyBytes is null or failure; or
        // - response's status is not an ok status,
        if (body_bytes.template has<Empty>() || body_bytes.template has<Fetch::Infrastructure::FetchAlgorithms::ConsumeBodyFailureTag>() || !Fetch::Infrastructure::is_ok_status(internal_response->status())) {
        ...
        auto potential_mime_type_for_encoding = Fetch::Infrastructure::extract_mime_type(internal_response->header_list());
        ...
        // FIXME: Pass options.
        auto response_url = internal_response->url().value_or({});
...
WebIDL::ExceptionOr<void> fetch_classic_worker_script(URL::URL const& url, EnvironmentSettingsObject& fetch_client, Fetch::Infrastructure::Request::Destination destination, EnvironmentSettingsObject& settings_object, PerformTheFetchHook perform_fetch, OnFetchScriptComplete on_complete)
{
    ...
    auto process_response_consume_body = [&settings_object, on_complete = move(on_complete)](auto response, auto body_bytes) {
        // Root response and internal_response so they survive any GC cycle
        // triggered by allocations later in this callback.
        auto response_root = GC::make_root(response);
        auto internal_response = response->unsafe_response();
        auto internal_response_root = GC::make_root(internal_response);
        ...
        auto maybe_mime_type = Fetch::Infrastructure::extract_mime_type(internal_response->header_list());
        auto mime_type_is_javascript = maybe_mime_type.has_value() && maybe_mime_type->is_javascript();

        if (internal_response->url().has_value() && Fetch::Infrastructure::is_http_or_https_scheme(internal_response->url()->scheme()) && !mime_type_is_javascript) {
            auto mime_type_serialized = maybe_mime_type.has_value() ? maybe_mime_type->serialized() : "unknown"_string;
            dbgln("Invalid non-javascript mime type \"{}\" for worker script at {}", mime_type_serialized, internal_response->url().value());
...
WebIDL::ExceptionOr<GC::Ref<ClassicScript>> fetch_a_classic_worker_imported_script(URL::URL const& url, HTML::EnvironmentSettingsObject& settings_object, PerformTheFetchHook perform_fetch)
{
    ...
    struct ResponseHolder {
        Optional<Fetch::Infrastructure::RootedResponseReferences> rooted_responses;
    ...
    auto process_response_consume_body = [&response_holder](GC::Ref<Fetch::Infrastructure::Response> res, Fetch::Infrastructure::FetchAlgorithms::BodyBytes bb) {
        // 1. Set bodyBytes to bb.
        response_holder.body_bytes = move(bb);

        // 2. Set response to res.
        response_holder.rooted_responses = Fetch::Infrastructure::root_response_references(res);
    };
    ...
    event_loop.spin_until(GC::create_function(vm.heap(), [&]() -> bool {
        return response_holder.rooted_responses.has_value();
    }));

    auto& rooted_responses = *response_holder.rooted_responses;
    auto public_response = rooted_responses->response();
    auto internal_response = rooted_responses->internal_response();
    auto body_bytes = move(response_holder.body_bytes);
    ...
    if (body_bytes.template has<Empty>() || body_bytes.template has<Fetch::Infrastructure::FetchAlgorithms::ConsumeBodyFailureTag>()
        || !Fetch::Infrastructure::is_ok_status(internal_response->status())
        || !Fetch::Infrastructure::extract_mime_type(internal_response->header_list()).has_value() || !Fetch::Infrastructure::extract_mime_type(internal_response->header_list())->is_javascript()) {
    ...
    auto muted_errors = public_response->is_cors_cross_origin() ? ClassicScript::MutedErrors::Yes : ClassicScript::MutedErrors::No;
    ...
    OnFetchScriptComplete on_complete)
{
    ...
    auto process_response_consume_body = [&module_map, url, module_type, &module_map_realm, on_complete](GC::Ref<Fetch::Infrastructure::Response> response, Fetch::Infrastructure::FetchAlgorithms::BodyBytes body_bytes) {
        // Root response so it survives any GC cycle triggered by
        // allocations later in this callback.
        auto response_root = GC::make_root(response);
