    ...
    for (size_t ip = 0; ip < bytecode_size;) {
        ...
        case OpCodeId::Repeat: {
            ...
            if (repeat_start > end_of_last_block)
                block_boundaries.append({ end_of_last_block, static_cast<size_t>(repeat_start), "Repeat"sv });
            block_boundaries.append({ static_cast<size_t>(repeat_start), ip, "Repeat after"sv });
            end_of_last_block = ip + sz;
            break;
        }
        default:
