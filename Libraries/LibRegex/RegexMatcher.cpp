    ...
    MatchState& state)
{
    ...
    case OpCodeId::SaveRightNamedCaptureGroup: {
        ...
        state.mutable_capture_group_matches(input.match_index).at(group_id - 1) = { view, static_cast<size_t>(name_index), input.line, start_position, input.global_offset + start_position };
        return { ExecutionResult::Continue, current_ip_size };
    }
