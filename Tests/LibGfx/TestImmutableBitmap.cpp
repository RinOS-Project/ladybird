...
#ifdef AK_OS_RINOS
TEST_CASE(rinos_export_to_byte_buffer_honors_webgl_packed_uploads)
{
    auto bitmap = MUST(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888,
        Gfx::AlphaType::Unpremultiplied, { 1, 1 }));
    bitmap->set_pixel(0, 0, Gfx::Color { 255, 128, 64, 128 });
    auto immutable_bitmap = Gfx::ImmutableBitmap::create(bitmap);

    auto expect_export = [&](Gfx::ExportFormat format, int flags,
                             Vector<u8> expected) {
        auto result = MUST(immutable_bitmap->export_to_byte_buffer(format,
            flags, 1, 1));
        EXPECT_EQ(result.buffer.bytes(), expected);
    };

    // No flag preserves straight-alpha source components. The same source
    // with UNPACK_PREMULTIPLY_ALPHA_WEBGL's export flag is converted before
    // each format's quantization, including the three packed WebGL types.
    expect_export(Gfx::ExportFormat::RGBA8888, 0,
        { 255, 128, 64, 128 });
    expect_export(Gfx::ExportFormat::RGBA8888,
        Gfx::ExportFlags::PremultiplyAlpha, { 128, 64, 32, 128 });
    expect_export(Gfx::ExportFormat::RGB565, 0, { 0x08, 0xfc });
    expect_export(Gfx::ExportFormat::RGB565,
        Gfx::ExportFlags::PremultiplyAlpha, { 0x04, 0x82 });
    expect_export(Gfx::ExportFormat::RGBA5551, 0, { 0x11, 0xfc });
    expect_export(Gfx::ExportFormat::RGBA5551,
        Gfx::ExportFlags::PremultiplyAlpha, { 0x09, 0x82 });
    expect_export(Gfx::ExportFormat::RGBA4444, 0, { 0x48, 0xf8 });
    expect_export(Gfx::ExportFormat::RGBA4444,
        Gfx::ExportFlags::PremultiplyAlpha, { 0x28, 0x84 });

    auto premultiplied_bitmap = MUST(Gfx::Bitmap::create(
        Gfx::BitmapFormat::BGRA8888, Gfx::AlphaType::Premultiplied, { 1, 1 }));
    premultiplied_bitmap->set_pixel(0, 0, Gfx::Color { 128, 64, 32, 128 });
    auto premultiplied_immutable_bitmap =
        Gfx::ImmutableBitmap::create(premultiplied_bitmap);
    auto unpremultiplied = MUST(premultiplied_immutable_bitmap
